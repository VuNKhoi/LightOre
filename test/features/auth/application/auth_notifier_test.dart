...
void main() {
  ...
  group('AuthNotifier', () {
    ...
    test('login() does not set state if repository returns false', () async {
      when(() => mockAuthRepository.setAuthenticated(any())).thenAnswer(
          (invocation) async =>
              invocation.positionalArguments[0] == true ? false : true);
      when(() => mockAuthRepository.isAuthenticated())
          .thenAnswer((_) async => null);
      container = createTestProviderContainer(mockAuthRepository);
      final notifier = container!.read(authProvider.notifier);
    ...
    test('logout() does not set state if repository returns false', () async {
      when(() => mockAuthRepository.setAuthenticated(any())).thenAnswer(
          (invocation) async =>
              invocation.positionalArguments[0] == false ? false : true);
      when(() => mockAuthRepository.signOut())
          .thenAnswer((_) async => Future.value());
      when(() => mockAuthRepository.isAuthenticated())
          .thenAnswer((_) async => null);
